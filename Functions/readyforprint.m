...
function readyforprint(wh,fs,fgcin,bgcin,lw,fh)
...
defaults.axes_props.FontSize = fs;
...
% specify line width
if ~isempty(lw)
	defaults.axes_props.LineWidth = lw;
...
defaults.legend_props.FontSize = fs;
...
defaults.title_props.FontSize = fs;
defaults.title_props.FontName = 'Helvetica';
defaults.title_props.FontWeight = 'bold';

% specify the label properties
defaults.label_props.Color =  fgc;
defaults.label_props.FontSize = max(fs,6);
defaults.label_props.FontSize = fs;
defaults.label_props.BackgroundColor = 'none';
defaults.label_props.FontName = 'Helvetica';
...
for myaxe = 1:length(myaxes)
    ...
    if ishandle(Ah)
        ...
        set(findobj(fh,'-regexp','FontName','[^'']'),'FontName','Helvetica');
